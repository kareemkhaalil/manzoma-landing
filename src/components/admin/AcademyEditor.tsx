  ...
  return (
    <div className="space-y-8 pb-20" dir="rtl">
      ...
      <div className="flex items-center justify-between sticky top-0 z-20 bg-slate-50/80 backdrop-blur-md py-4 border-b border-slate-200 -mx-4 px-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">أكاديمية منظومة</h1>
          <p className="text-sm font-bold text-slate-500 mt-1">إدارة دروس الفيديو والمحتوى التعليمي</p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {localConfig.academyVideos.map((video, idx) => (
          <div key={video.id} className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
            <div className="px-7 py-5 border-b border-slate-200 flex items-center justify-between bg-slate-50/30">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-slate-900 text-sm truncate max-w-[200px]">{video.title}</h3>
              </div>
              <button
            ...
            <div className="p-7 space-y-5">
              ...
              <div className="space-y-2">
                ...
                <div className="aspect-video bg-slate-50/50 rounded-2xl overflow-hidden border border-slate-200 relative group">
                  {video.url ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center gap-3 text-slate-500">
                      ...
                      <span className="text-[10px] font-bold uppercase tracking-widest">No Video URL</span>
                    </div>
                  )}
                </div>
              </div>
