    ...
    <section id="pricing" className="py-24 md:py-32 bg-brand-bg relative z-10">
      <div className="max-w-6xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 max-w-[1400px] mx-auto">
            ...
            >
              ...
              <div className="mb-6">
                ...
                {yearly && pkg.yearlyPrice && (
                  <p className={`text-xs font-bold mt-2 ${pkg.isPopular ? "text-brand-emerald-light" : "text-brand-emerald"}`}>
                    وفر {Math.round((1 - parseInt(pkg.yearlyPrice.replace(/,/g, '')) / parseInt(pkg.price.replace(/,/g, ''))) * 100)}% بالدفع السنوي
                  </p>
                )}
              </div>
              ...
              <Link
                to="/signup"
                className={`w-full py-3.5 rounded-full text-sm font-[800] transition-all duration-300 mb-6 flex items-center justify-center ${
                  pkg.isPopular
          ...
          {/* Hardcoded Enterprise Tier */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: visiblePackages.length * 0.1, type: "spring", damping: 20 }}
            className="relative rounded-3xl p-7 md:p-8 border bg-white border-brand-border hover:border-brand-primary/30 shadow-premium hover:shadow-premium-lg transition-all duration-300 flex flex-col h-full"
          >
            <div className="mb-6 flex-grow">
              <h3 className="text-xl font-[900] mb-2 text-brand-navy">
                باقة المؤسسات الكبرى
              </h3>
              <p className="text-sm font-medium text-brand-muted">
                للسلاسل التجارية الكبيرة والمشاريع التي تتطلب حلول مخصصة وبنية تحتية خاصة.
              </p>
            </div>

            <div className="mb-6">
              <span className="text-3xl font-[900] tracking-tight text-brand-navy">
                تسعير مخصص
              </span>
            </div>

            <a href="https://wa.me/201012345678" target="_blank" rel="noopener noreferrer" className="w-full py-3.5 rounded-full text-sm font-[800] transition-all duration-300 mb-6 bg-slate-100 text-brand-navy hover:bg-slate-200 flex items-center justify-center">
              تواصل مع فريق المبيعات
            </a>

            <div className="space-y-3">
              {[
                "سيرفرات خاصة (Dedicated)",
                ...
                <div key={fIdx} className="flex items-start gap-3">
                  <div className="w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 bg-brand-emerald/10 text-brand-emerald">
                    <Check className="w-3 h-3" />
                  </div>
