  ...
  return (
    <div className="space-y-8 pb-20" dir="rtl">
      ...
      <div className="flex items-center justify-between sticky top-0 z-20 bg-brand-bg/80 backdrop-blur-md py-4 border-b border-brand-border/20 -mx-4 px-4 mb-6">
        <div>
          <h1 className="text-2xl font-[900] text-brand-navy">محرر الصفحة الرئيسية</h1>
          <p className="text-sm font-bold text-brand-muted mt-1">تخصيص محتوى وهيكل الواجهة الأمامية</p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card title="قسم الهيرو (Hero)" icon={Layout}>
          ...
          <div className="space-y-2">
            <label className="text-xs font-[900] text-brand-muted px-1">الوصف الفرعي</label>
            <textarea
              rows={3}
              value={localConfig.hero.subtitle}
              onChange={(e) => setLocalConfig({ ...localConfig, hero: { ...localConfig.hero, subtitle: e.target.value } })}
              className="w-full bg-brand-bg/50 border border-brand-border/40 rounded-2xl px-5 py-3.5 text-sm font-bold text-brand-navy focus:bg-white focus:ring-2 focus:ring-brand-primary/10 focus:border-brand-primary outline-none transition-all resize-none"
            />
          </div>
        ...
        <Card title="الهوية والنمط البصري" icon={Palette}>
          ...
          <div className="flex items-center gap-6 pt-2">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm font-bold text-brand-navy">خلفية Mesh</span>
            </label>
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-sm font-bold text-brand-navy">توهج (Glow)</span>
            </label>
          </div>
        </Card>
          ...
          <Card
            ...
            action={
              <button
                ...
                className="bg-white border border-brand-border/40 text-brand-primary px-4 py-2 rounded-xl font-[900] text-xs hover:bg-brand-primary hover:text-white transition-all"
              >
                + إضافة ميزة
              </button>
            }
          >
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
              {localConfig.features.map((feat, idx) => (
                <div key={feat.id} className="bg-brand-bg/30 border border-brand-border/20 rounded-2xl p-5 relative group">
                  <button
          ...
          <Card title="شركاء النجاح" icon={Image}>
            <div className="flex flex-wrap gap-2">
              {localConfig.partners.map((partner, idx) => (
                <div key={idx} className="flex items-center gap-2 bg-white border border-brand-border/40 px-3 py-1.5 rounded-xl">
                  <span className="text-sm font-bold text-brand-navy">{partner}</span>
                  <button onClick={() => setLocalConfig({ ...localConfig, partners: localConfig.partners.filter((_, i) => i !== idx) })} className="text-brand-muted hover:text-rose-500 transition-colors">
              ...
              <input
                ...
                className="bg-brand-bg/50 border border-brand-border/40 rounded-xl px-4 py-1.5 text-sm font-bold text-brand-navy outline-none focus:bg-white w-40 transition-all"
              />
            </div>
          </Card>
        </div>
